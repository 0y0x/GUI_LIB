...
run(function()
			...
			if callback then
				repeat
					...
					if mode == 'Tool' then
						local tool = getTool()
						if tool then
							-- Always activates tool if Auto is on, or if holding LMB when Auto is off
							if Auto.Enabled or inputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton1) then
								tool:Activate()
							end
						end
					elseif mode == 'Click' or mode == 'RightClick' then
						if (isrbxactive or iswindowactive)() and not vape.gui.ScaledGui.ClickGui.Visible then
							if Auto.Enabled or inputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton1) then
								if mode == 'Click' then
									mouse1click()
								else
	...
	Mode = AutoClicker:CreateDropdown({
		...
		Tooltip = 'Tool - Automatically uses Roblox tools (e.g., swords)\nClick - Left click\nRightClick - Right click'
	})
	...
	Auto = AutoClicker:CreateToggle({
		Name = 'Auto',
		Tooltip = 'When enabled, it clicks automatically without holding the mouse button.'
	})
end)
...
run(function()
	...
	AllWeapon = vape.Categories.Blatant:CreateModule({
		Name = 'AllWeapon',
		Function = function(callback)
			local Players = game:GetService("Players")
			local player = Players.LocalPlayer

			-- Shared state so it persists between toggles
			if not getgenv().AllWeaponData then
				getgenv().AllWeaponData = {
					running = false,
					equipThread = nil
				}
			end

			local data = getgenv().AllWeaponData

			local TOOL_NAMES = {
				"Axe of Skirmishing",
				"Battle Cleaver",
				"Fine Steel Sword",
				"Greatsword of Flying",
				"Greatsword of Flying II",
				"Iceblade",
				"Iron Sword",
				"Power Katana",
				"Rusty Iron Sword",
			...
			local function equipSpecificTools()
			...
			if callback then
				...
				data.equipThread = task.spawn(function()
				...
				-- Disable auto-equip and unequip everything
				data.running = false
				unequipAllTools()
			end
		end,
		Tooltip = 'Automatically equips all your melee weapons from your inventory.'
	})
end)
...
run(function()
    ...
    CustomJump = vape.Categories.Blatant:CreateModule({
            ...
            if callback then
                ...
                jumpConnection = UserInputService.JumpRequest:Connect(function()
                    ...
                    elseif Mode.Value == "Velocity" then
                        char.HumanoidRootPart.Velocity += Vector3.new(0, Velocity.Value, 0)
                    end
                end)

                -- Continuous boost while in air for Velocity mode
                boostConnection = RunService.RenderStepped:Connect(function(dt)
                    local char = entityLibrary.character
                    if Mode.Value == "Velocity" and entityLibrary.isAlive and char and char.Humanoid.FloorMaterial == Enum.Material.Air then
                        char.HumanoidRootPart.Velocity += Vector3.new(0, Velocity.Value * dt, 0)
                    end
                end)
            else
                -- Disconnect everything when module is toggled off
        ...
        Tooltip = "Custom jump module"
    })

    Mode = CustomJump:CreateDropdown({
        Name = "Mode",
        List = {"Normal", "Velocity"},
        Function = function() end
    })

    Velocity = CustomJump:CreateSlider({
        Name = "Velocity",
        Min = 1,
        Max = 100,
        Default = 50,
